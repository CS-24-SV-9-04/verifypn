...
int explicitColored(options_t& options, shared_string_set& string_set, std::vector<Condition_ptr>& queries, const std::vector<std::string>& queryNames) {
    ...
    auto variableMap = builder.takeVariableMap();
    ...
    for (size_t i = 0; i < queries.size(); i++) {
        ...
        ExplicitColored::NaiveWorklist naiveWorkList(net, queries[i], placeIndices, resultPrinter, variableMap, transitionIndices);
