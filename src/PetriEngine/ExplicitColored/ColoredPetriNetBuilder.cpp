...
namespace PetriEngine {
    namespace ExplicitColored {
        ...
        std::unordered_map<std::string, uint32_t> ColoredPetriNetBuilder::takeTransitionIndices() {
            return std::move(_transitionIndices);
        }
        std::unordered_map<std::string, Variable_t> ColoredPetriNetBuilder::takeVariableMap() const {
            return *_variableMap;
        }
    }
}